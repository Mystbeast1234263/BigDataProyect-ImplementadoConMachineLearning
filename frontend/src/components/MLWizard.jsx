...
export default function MLWizard({ sensorType, dateFilter = null }) {
  ...
  const checkDataAvailability = async () => {
    ...
    try {
      ...
      const totalRecords = stats.count || 0;
        ...
        {currentStep === 4 && (
          <div>
            {modelMetrics ? (
              <>
                <div className="mb-6">
                  <h5 className="text-lg font-semibold text-white mb-4">Métricas del Modelo</h5>
                  ...
                  <>
                    ...
                    <div className="mb-6 bg-slate-800 rounded-lg p-4 border border-slate-700">
                      ...
                      <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-slate-900/60 rounded-lg p-4 border border-slate-700">
                          <p className="text-xs text-slate-400 mb-1">Total de predicciones</p>
                          <p className="text-white font-bold text-2xl">{predictions.total_predictions}</p>
                        </div>
                          ...
                          <div className="mb-6 bg-slate-900/40 rounded-lg p-4 border border-slate-700">
                            ...
                            {filteredPredictions.length > 0 ? (
                              <ResponsiveContainer width="100%" height={400}>
                                <LineChart 
                                  data={filteredPredictions.slice(0, 100).map((pred, idx) => {
                                    const dateStr = pred.timestamp 
                                      ? new Date(pred.timestamp).toLocaleDateString('es-ES', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
                                      : pred.prediction_date 
                                        ? new Date(pred.prediction_date).toLocaleDateString('es-ES', { month: 'short', day: 'numeric' })
                                        : `${pred.prediction_month || ''} #${idx + 1}`;
                                    return {
                                      fecha: dateStr,
                        ...
                        <div className="mb-6 bg-slate-900/40 rounded-lg p-4 border border-slate-700">
                          ...
                          <div className="grid grid-cols-3 gap-4">
                            {['normal', 'warning', 'critical'].map((cls) => {
                              const count = predictions.predictions?.filter(p => p.predicted_class === cls).length || 0;
                              const percentage = predictions.total_predictions > 0 ? (count / predictions.total_predictions * 100).toFixed(1) : 0;
                        ...
                        <table className="min-w-full divide-y divide-slate-700">
                          ...
                          <tbody className="divide-y divide-slate-800">
                            {(() => {
                              ...
                              return filteredPredictions.slice(0, 50).map((pred, idx) => (
                                <tr key={idx} className="hover:bg-slate-700/50 transition-colors">
                                  <td className="px-4 py-3 text-slate-300 text-xs">
                                    {pred.timestamp ? new Date(pred.timestamp).toLocaleString() : pred.prediction_date || pred.prediction_month || 'N/A'}
                                  </td>
