...
class MongoDBService:
    ...
    def get_sensor_stats(
        ...
        try:
            ...
            # Add total count and date range info
            ...
            if documents:
                ...
                if timestamps:
                    stats["min_date"] = min(timestamps).isoformat() if hasattr(min(timestamps), 'isoformat') else str(min(timestamps))
                    stats["max_date"] = max(timestamps).isoformat() if hasattr(max(timestamps), 'isoformat') else str(max(timestamps))
